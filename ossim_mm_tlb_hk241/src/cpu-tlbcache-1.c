...
int tlb_cache_read(struct memphy_struct * mp, int pid, int pgnum, int* value)
{
   ...
   for(int j = 3; j >= 0; j--)
   {
      entry_pid |= (mp->storage[base_entry_addr + 4 - j] << (j * 8));
   }
   int entry_pgnum = (mp->storage[base_entry_addr + 5] << 8) | mp->storage[base_entry_addr + 6];
   int entry_data = (mp->storage[base_entry_addr + 7] << 8) | mp->storage[base_entry_addr + 8]; // Frame number
